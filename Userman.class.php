...
class Userman extends FreePBX_Helpers implements BMO {
	...
	*/
	private function getUserModulesSettingBasedOnTemplate($widget,$userid,$tempid){
		...
		if(method_exists($this->FreePBX->$moduleuc,'getWidgetListByModule')){
			...
			dbug('getWidgetListByModule method doesnot exist in '.$moduleuc);
		}
		return $widget;
	}
	...
	*/
	public function addTemplateSettings($tempid,$uid){dbug('addTemplateSettings');
		...
		if(is_array($results)) {
			...
			foreach($results as $result){dbug($result);
	...
	*/
	...
	/* get an unused extension to associate with Template Creator*/
	private function getUnUsedextension(){
		$cul = array();
		foreach($this->FreePBX->Core->listUsers() as $list) {
			$cul[$list[0]] = array(
				"name" => $list[1],
		...
		foreach($cul as $e => $u) {
			...
			$dfpbxusers[] = array("ext" => $e, "name" => $u['name'], "selected" => false);
		}
		//return the last entry from array
		return end($dfpbxusers);
	}
